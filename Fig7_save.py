...
# print("Fig. 7.5 data shape : ", data.shape) # 7.3
# with open("./plot_data/{}_75.pkl".format(monkey), "wb") as file:
#     pickle.dump(data, file)
# print("="*50)


# Fig. 7.3 (out of vague)
print("For Fig. 7.3 : ")

vague_index = [
            list(i)
            for i in consecutive_groups(
            df_reset_comb[df_reset_comb.labels == "vague"].index
        )
]
around_vague_index = []
for each in vague_index:
    if len(each) == 0:
        continue
    around_vague_index.extend(list(
        np.arange(max(0, each[0]-5), each[0])
    )) # before vague
    around_vague_index.extend(list(
        np.arange(each[-1]+1, min(each[-1]+6, df_reset_comb.shape[0]))
    ))  # after vague
print("Length of around vague data : ", len(around_vague_index))


vague_df = (
    df_reset_comb.iloc[around_vague_index]
    .filter(regex="_weight")
    .stack()
    .sort_values(ascending=False)
    .reset_index()
