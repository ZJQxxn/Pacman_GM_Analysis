...
class FractionFeatureGHAnalyzer:
    '''
    Description:
    ...
    def _G2HPreprocess(self, need_stan=False):
        ...
        for index, each in enumerate(self.data[["file", "index", "distance1", "distance2","rwd_pac_distance"]].values):
            ...
            temp = [
                # 1 / ( 1 + np.exp( - combined_dist / dot_distance)), # use combined_dist / dot_dist as feature; normalization
                combined_dist / dot_distance, # use combined_dist / dot_dist as feature
    ...
    def G2HAnalyzeDTree(self):
        ...
        graph.render('G2H_fraction_feature_trained_tree_structure', view=False)
        # Collect data
        testing_data = pd.DataFrame(testing_data)
        testing_data.columns = ['comb_dot_frac', 'nearby_bean_num']
        ...
        testing_result = testing_result.assign(
            bin_comb_dot_frac = pd.cut(testing_result.comb_dot_frac, bins=30).astype(str).apply(
...
if __name__ == '__main__':
    ...
    a = FractionFeatureGHAnalyzer(feature_filename, label_filename, mode_filename)
