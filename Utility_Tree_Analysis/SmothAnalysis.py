'''
Description:
    Fit smooth estimations based on Bayesian model.
...
def negativeLikelihood(param, all_data, true_prob, agents_list, prev_weight, smooth_par, sparse_par, return_trajectory = False, need_intercept = False):
    '''
    ...
    if prev_weight is not None:
        nll = nll + smooth_par * np.linalg.norm(np.array(agent_weight) - np.array(prev_weight))
    # Add sparse constraint
    nll = nll + sparse_par * np.linalg.norm(np.array(agent_weight), ord = 1)
...
def constraintMLE(config):
    print("=" * 20, " Single Trial (Fitting w/ Constraints) ", "=" * 20)
    # Read trial data
    agents_list = ["{}_Q".format(each) for each in
                   ["global", "local", "pessimistic_blinky", "pessimistic_clyde", "suicide", "planned_hunting"]]
    ...
    agent_name = ["global", "local", "pessimistic_blinky", "pessimistic_clyde", "suicide", "planned_hunting"]
    agent_index = [["global", "local", "pessimistic_blinky", "pessimistic_clyde", "suicide", "planned_hunting"].index(i)
                   for i in agent_name]
    # Construct optimizer
    for trial_index, each in enumerate(trial_data):
        # if trial_index > 50:
        ...
        plt.title("Gini Coefficient : ({b:.3f})".format(
            b=np.nanmean(trial_gini) if not np.all(np.isnan(trial_gini)) else np.nan),
                  fontsize=10)
...
def cleanMLE(config):
    print("=" * 20, " Single Trial (Smooth Fitting) ", "=" * 20)
    # Read trial data
    agents_list = ["{}_Q".format(each) for each in
                   ["global", "local", "pessimistic_blinky", "pessimistic_clyde", "suicide", "planned_hunting"]]
    window = config["single_trial_window"]
...
def _parameterPari(window_size_list, alpha_list, beta_list):
    res = []
...
def MLEwithBIC(config):
    print("=" * 20, " Single Trial (BIC) ", "=" * 20)
    window_size_list = [2, 3, 4, 5, 6, 7]
    alpha_list = [0.01, 0.05, 0.1, 0.5]
    beta_list = [0.01, 0.05, 0.1, 0.5]
    paramet_pairs  =_parameterPari(window_size_list, alpha_list, beta_list)
    # Read trial data
    agents_list = ["{}_Q".format(each) for each in
                   ["global", "local", "pessimistic_blinky", "pessimistic_clyde", "suicide", "planned_hunting"]]
    window = config["single_trial_window"]
    ...
    for trial_index, each in enumerate(trial_data):
        ...
        k = len(agent_name)+1
        n = trial_length
        print("The number of parameter pairs : ", len(paramet_pairs))
        for par in range(len(bounds)):
            l = {'type': 'ineq', 'fun': lambda x: x[par] - bounds[par][0]}
            u = {'type': 'ineq', 'fun': lambda x: bounds[par][1] - x[par]}
            cons.append(l)
            cons.append(u)
        for pair_index, pair in enumerate(paramet_pairs):
            # window = pair[0]
            alpha = pair[0]
            beta = pair[1]
            # estimation in the window
            ...
            BIC = k*np.log(n)+nll
            BIC_list.append(BIC)
            print("|{} pair| BIC = {}".format(pair_index, BIC))
        best_pair = np.argsort(BIC_list)[0]
        alpha =  paramet_pairs[best_pair][0]
        beta = paramet_pairs[best_pair][1]
        print("Best Alpha = {} | Best Beta = {} | Best BIC = {}".format(alpha, beta, BIC_list[best_pair]))
