...
def _ghostProcesing(pess_Q, PG, ghost_status):
    if np.any(np.concatenate(pess_Q) < 0):
        temp_pess_Q = np.concatenate(pess_Q)
        temp_pess_Q[temp_pess_Q > 0] = 0.0
        offset = np.max(np.abs(temp_pess_Q))  # TODO: max absolute value of negative values
...
def readTrialData(filename):
        ...
        if isinstance(all_data.global_Q[index], list):  # what if no normal Q?
            if index == all_data.shape[0] - 1:
                if isinstance(all_data.global_Q[index - 1], list):
                    ...
                    all_data.blinky_Q[index] = all_data.blinky_Q[index - 2]
                    all_data.clyde_Q[index] = all_data.clyde_Q[index - 2]
                    ...
                    all_data.blinky_Q[index] = all_data.blinky_Q[index - 1]
                    all_data.clyde_Q[index] = all_data.clyde_Q[index - 1]
    ...
    all_data.blinky_Q = _ghostProcesing(all_data.blinky_Q, PG, ghost_status)
    all_data.clyde_Q = _ghostProcesing(all_data.clyde_Q, PG, ghost_status)
...
def multiAgentAnalysis(config):
    ...
    agent_index = [["global", "local", "blinky", "clyde", "planned_hunting"].index(each) for each in agent_name]
    ...
    np.save("../common_data/{}/ghost-{}-window{}-{}_intercept-multi_labels.npy".format(
        dir_names, save_base, window, "w" if config["need_intercept"] else "wo"), all_estimated_label)
    np.save("../common_data/{}/ghost-{}-window{}-{}_intercept-handcrafted_labels.npy".format(
        dir_names, save_base, window, "w" if config["need_intercept"] else "wo"), handcrafted_labels)
    np.save("../common_data/{}/ghost-{}-window{}-{}_intercept-matching_rate.npy".format(
        dir_names, save_base, window, "w" if config["need_intercept"] else "wo"), trial_matching_rate)
    np.save("../common_data/{}/ghost-{}-window{}-{}_intercept-trial_weight.npy".format(
        dir_names, save_base, window, "w" if config["need_intercept"] else "wo"), trial_weight)
    np.save("../common_data/{}/ghost-{}-window{}-{}_intercept-Q.npy".format(
        dir_names, save_base, window, "w" if config["need_intercept"] else "wo"), trial_Q)
    np.save("../common_data/{}/ghost-{}-window{}-{}_intercept-contribution.npy".format(
    ...
    if "new" in config["trial_data_filename"]:
        ...
        data_type = None
    np.save("../common_data/{}/{}_ghost_records.npy".format(dir_names, data_type), record)
...
def incrementalAnalysis(config):
    ...
    incremental_agents_list = [
        ...
        ["local", "blinky", "global"],
        ["local", "clyde", "global"],
        ["local", "blinky", "clyde", "global"],
        ["local", "blinky", "clyde", "global", "planned_hunting"],
    ]
    all_cr = []
    for trial_index, each in enumerate(trial_data):
        print("-"*15)
    ...
    np.save("../common_data/incremental/ghost-window{}-incremental_cr-{}_intercept.npy".format(
...
def decrementalAnalysis(config):
    ...
    incremental_agents_list = [
        ["local", "blinky", "clyde", "planned_hunting"], # w/o global
        ["global", "blinky", "clyde", "planned_hunting"], # w/o local
        ["global", "local", "clyde", "planned_hunting"], # w/o blinky
        ["global", "local", "blinky", "planned_hunting"],  # w/o clyde
        ["global", "local", "blinky", "clyde"], # w/o planned hunting
        ["global", "local", "blinky", "clyde", "planned_hunting"] # all the agents
    ]
    all_cr = []
    for trial_index, each in enumerate(trial_data):
        print("-"*15)
    ...
    np.save("../common_data/decremental/ghost-{}trial-window{}-incremental_cr-{}_intercept.npy".format(
...
def oneAgentAnalysis(config):
    ...
    np.save("../common_data/one_agent/ghost-{}trial-window{}-incremental_cr-{}_intercept.npy".format(
        config["incremental_num_trial"], window, "w" if config["need_intercept"] else "wo"), all_cr)
...
def stageCombineAnalysis(config):
    ...
    incremental_agents_list = [
        ...
        ["clyde"],
        ["planned_hunting"],
        ["global", "local", "blinky", "clyde", "planned_hunting"]
    ]
    all_cr = []
    weight = []
    for agent_name in incremental_agents_list:
    ...
    filename = "../common_data/stage_together/ghost-all-100trial-cr.npy"
    np.save(filename, all_cr)
    np.save("../common_data/stage_together/ghost-all-100trial-weight.npy", weight)
...
def specialCaseAnalysis(config):
    ...
    filename = "../common_data/special_case/ghost-100trial-cr.npy"
    np.save(filename, cr_trial)
    np.save("../common_data/special_case/ghost-100trial-contribution.npy", cr_contribuion)
...
def specialCaseMovingAnalysis(config):
    ...
    if "special_case" not in os.listdir("../common_data"):
        os.mkdir("../common_data/special_case")
    filename = "../common_data/special_case/ghost-100trial-moving_window-cr.npy"
    np.save(filename, cr_trial)


def diffLabelAnalysis():
    print("="*20, " Diff State Analysis ", "="*20)
    filename = "../common_data/trial/100_trial_data_Omega-with_Q-ghost.pkl"
    print(filename)
    ...
    label_list = ["label_local_graze", "label_local_graze_noghost", "label_global_ending",
                  ...
                  "label_true_planned_hunting"]
    agents_list = ["{}_Q".format(each) for each in ["global", "local", "blinky", "clyde", "planned_hunting"]]
    agent_name_list = [["local"], ["global", "local", "blinky", "clyde", "planned_hunting"]]
    local_cr = []
    global_cr = []
    evade_cr = []
    suicide_cr = []
    ...
    np.save("../common_data/state_comparison/ghost-100trial_Omega_diff_state_agent_cr.npy", state_cr)


def diffFittedLabelAnalysis():
    print("="*20, " Diff State Analysis (Fitted Label) ", "="*20)
    filename = "../common_data/trial/100_trial_data_Omega-with_Q-ghost.pkl"
    print(filename)
    data = readTrialData(filename)
    # data = [data[i] for i in range(10)]
...
def ghostLabelAnalysis(config):
    print("="*20, " Ghost Label Analysis ", "="*20)
    # Read trial data
    agents_list = ["{}_Q".format(each) for each in ["global", "local", "blinky", "clyde", "planned_hunting"]]
    ...
    for trial_index, each in enumerate(trial_data):
        ...
        for centering_index, centering_point in enumerate(window_index):
            ...
            temp_contribution[centering_index, :] = minmax_scale(contribution, feature_range=(-1.0, 1.0))
    ...
    # Save data
    if "ghost_label_analysis" not in os.listdir("../common_data"):
        os.mkdir("../common_data/ghost_label_analysis")
    np.save(
        "../common_data/ghost_label_analysis/{}-record.npy".format(
...
if __name__ == '__main__':
    ...
    config = {
        ...
        # Filename
        # "trial_data_filename": "../common_data/trial/{}_100_trial_data_Omega-with_Q-ghos.pkl".format(type),
        "trial_data_filename": "../common_data/trial/100_trial_data_Omega-with_Q-ghost.pkl".format(type),
        # The number of trials used for analysis
        "trial_num" : None,
        # Window size for correlation analysis
        "trial_window" : 3,
        "multi_agent_list" : ["global", "local", "blinky", "clyde", "planned_hunting"],
        # ==================================================================================
        "incremental_window" : 3,
        "incremental_data_filename" : "../common_data/trial/100_trial_data_Omega-with_Q-ghost.pkl",
        "incremental_num_trial" : None,

        "single_trial_data_filename" : "../common_data/trial/test_suicide_trial_data_Omega-with_Q-ghost.pkl",
        "single_trial_window" : 3,

        # ==================================================================================
        # "descriptive_filename" : "../common_data/trial/accidental_200_trial_data_Omega-with_Q-descriptive.pkl",
        "descriptive_filename" : "../common_data/trial/100_trial_data_Omega-with_Q-ghost.pkl",
    ...
    ghostLabelAnalysis(config)

    # a = np.array([-3.5, 0.4, 1.9, -2.2, 3.0])
    #
    # b = minmax_scale(a, feature_range=(-1, 1))
    # print()