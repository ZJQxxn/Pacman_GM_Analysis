...
from EqualAnalysis import readTrialData, negativeLikelihood, _makeChoice
...
def perceptron(config, feature_type):
    ...
    np.save("../common_data/LR_comparison/equal-{}-window{}-perceptron-{}-weight.npy".format(save_base, window, feature_type), all_weight)
    np.save("../common_data/LR_comparison/equal-{}-window{}-perceptron-{}-cr.npy".format(save_base, window, feature_type), all_cr)
...
def multiAgentAnalysis(config):
    ...
    np.save("../common_data/LR_comparison/equal-{}-window{}-agent-{}-cr.npy".format(save_base, window, "_".join(agent_name)), trial_cr)
    # np.save("../common_data/LR_comparison/{}-window{}-agent-{}-Q.npy".format(save_base, window, "_".join(agent_name)), trial_Q)
    np.save("../common_data/LR_comparison/equal-{}-window{}-agent-{}-contribution.npy".format(save_base, window, "_".join(agent_name)), trial_contribution)
...
def showDistPlot():
    ...
    perceptron_cr = np.load(
        "../common_data/LR_comparison/equal-100_trial_data_{}-with_Q-equal-window5-perceptron-features_all-cr.npy".format(
            type), allow_pickle=True)
    perceptron_local_cr = np.load(
        "../common_data/LR_comparison/equal-100_trial_data_{}-with_Q-equal-window5-perceptron-features_local-cr.npy".format(
            type), allow_pickle=True)
    perceptron_dir_cr = np.load(
        "../common_data/LR_comparison/equal-100_trial_data_{}-with_Q-equal-window5-perceptron-features_wrt_dir-cr.npy".format(
            type), allow_pickle=True)
    multi_agent_cr = np.load(
        "../common_data/LR_comparison/equal-100_trial_data_{}-with_Q-equal-window5-agent-global_local_pessimistic_blinky_pessimistic_clyde_suicide_planned_hunting-cr.npy".format(
            type), allow_pickle=True)
    local_cr = np.load(
        "../common_data/LR_comparison/equal-100_trial_data_{}-with_Q-equal-window5-agent-local-cr.npy".format(
            type), allow_pickle=True)
    perceptron_trial_cr = [np.nanmean(each) for each in perceptron_cr]
    perceptron_local_trial_cr = [np.nanmean(each) for each in perceptron_local_cr]
    perceptron_dir_trial_cr = [np.nanmean(each) for each in perceptron_dir_cr]
...
if __name__ == '__main__':
    config = {
        ...
        "data_filename": "../common_data/trial/100_trial_data_Omega-with_Q-equal.pkl",
        "window" : 5,
        "trial_num" : None,
        "need_intercept" : True,
    ...
    # comparison(config)
    # config["data_filename"] = "../common_data/trial/100_trial_data_Patamon-with_Q-equal.pkl"
    # comparison(config)
    #
    ...
    # comparison(config)
    #
    # config["data_filename"] = "../common_data/trial/100_trial_data_Omega-with_Q-equal.pkl"
    # comparison(config)
