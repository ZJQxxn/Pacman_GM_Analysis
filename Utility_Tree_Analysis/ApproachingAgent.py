...
class ApproachingAgent:
    ...
    def nextDir(self, return_Q = False):
        ...
        for index, each in enumerate(suicide_direction):
            temp_utility = 0.0
            ghost_repulsive_thr = 15 #TODO: as parameter
            P_G = P_G_distance[index]
            R = self.reward_amount[8]
            T = ghost_repulsive_thr
            if P_G <= ghost_repulsive_thr:
                temp_utility += (-R / T) * P_G + R
            if self.Q_value[self.dir_list.index(each)] != 0.0:
                self.Q_value[self.dir_list.index(each)] = max(temp_utility, self.Q_value[self.dir_list.index(each)])
            else:
                self.Q_value[self.dir_list.index(each)] = temp_utility
