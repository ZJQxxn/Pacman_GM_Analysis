...
from MultiAgent_Analysis.GhostPessimisticAgent import GhostPessimistic
import time
...
class MultiAgentInteractor:
    ...
    def __init__(self, config_file):
        ...
        # Determine whether the game status is reset after the last estimation. Notice that the game status should be
        ...
        self.ghost1_pessimistic_agent = None
        self.ghost2_pessimistic_agent = None
    ...
    def _ghost1pessimisticAgent(self):
        '''
        Use the global agent to predict the moving direction given game status of the current time step. 
        :return: The one-hot vector denoting the direction estimation of global agent.
        '''
        # Construct the decision tree for the ghost 1 pessimistic agent
        self.ghost1_pessimistic_agent = GhostPessimistic(
            self.adjacent_data,
            self.locs_df,
            self.reward_amount,
            self.cur_pos,
        ...
        # Estimate the moving direction
        pessimistic_result = self.ghost1_pessimistic_agent.nextDir(return_Q = True)
        pessimistic_Q = pessimistic_result[1]
        return pessimistic_Q


    def _ghost2pessimisticAgent(self):
        '''
        Use the global agent to predict the moving direction given game status of the current time step.
        :return: The one-hot vector denoting the direction estimation of global agent.
        '''
        # Construct the decision tree for the ghost 1 pessimistic agent
        self.ghost2_pessimistic_agent = GhostPessimistic(
            self.adjacent_data,
            self.locs_df,
            self.reward_amount,
            self.cur_pos,
        ...
        # Estimate the moving direction
        pessimistic_result = self.ghost2_pessimistic_agent.nextDir(return_Q = True)
        pessimistic_Q = pessimistic_result[1]
        return pessimistic_Q
    ...
    def estimateDir(self):
        ...
        agent_estimation[:, 2] = self._ghost1pessimisticAgent()
        agent_estimation[:, 3] = self._ghost2pessimisticAgent()
