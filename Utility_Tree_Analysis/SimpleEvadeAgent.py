    ...
    def _countGhost(self, upper_left, lower_right):
        ...
        for i in range(upper_left[0], lower_right[0]+1):
            for j in range(upper_left[1], lower_right[1]+1):
                area_loc.append((i,j))
        if isinstance(self.ghost_status, float) or isinstance(self.ghost_data, float) or self.ghost_status >= 3:
            return 0
        else:
            if self.ghost_data in area_loc:
                return -1
            return 0
