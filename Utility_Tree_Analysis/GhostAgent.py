...
class GhostAgent:
    ...
    def __init__(self, adjacent_data, adjacent_path, locs_df, reward_amount, root, energizer_data, bean_data, ghost_data, reward_type,
                 fruit_pos, ghost_status, last_dir, ghost_name,
        ...
        # Ghost name (red or yellow)
        self.ghost_name = ghost_name
        if self.ghost_name == "red":
            self.ghost_status = self.ghost_status[0]
            self.ghost_data = self.ghost_data[0]
        elif self.ghost_name == "yellow":
            self.ghost_status = self.ghost_status[1]
            self.ghost_data = self.ghost_data[1]
        else:
            raise ValueError("Undefined ghost name {}!".format(self.ghost_name))
        # Last direction
    ...
    def _countGhost(self, upper_left, lower_right):
        area_loc = []
        # Construct a grid area
        for i in range(upper_left[0], lower_right[0]+1):
            for j in range(upper_left[1], lower_right[1]+1):
                area_loc.append((i,j))
        if np.all(np.array(self.ghost_data) <= 3):
            return 0
        else:
            ghost_num = 0
            if self.ghost_status != 3 and self.ghost_data in area_loc:
                ghost_num += 1
            return ghost_num
    ...
    def nextDir(self, return_Q=False):
        ...
        for dir in self.available_dir:
            ...
            ghost_num = self._countGhost(area[0], area[1])
            self.Q_value[self.dir_list.index(dir)] = ghost_num
...
if __name__ == '__main__':
    ...
    print("Blinky Agent Choice : ", choice, Q)
    ...
    _, Q = agent.nextDir(return_Q=True)
    choice = agent.dir_list[makeChoice(Q)]
    print("Clde Agent Choice : ", choice, Q)
